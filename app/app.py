...
def index():
    ...
    if len(request.args) > 1 or (len(request.args) == 1 and not page):
        books_list, pages_count = usecase.search_books(db, int(page), request.args.get('title'), request.args.getlist('genres'), request.args.getlist('years'), request.args.get('volume_from'), request.args.get('volume_to'), request.args.get('author'))
